...
describe('JulesTools', () => {
  ...
  describe('executeWithErrorHandling', () => {
    ...
    it('Arbitrary Error returns generic message', async () => {
      vi.mocked(client.createSession).mockRejectedValue(new Error('Internal Database Error'));
    ...
    it('console.error is called with just the message string', async () => {
      ...
      vi.mocked(client.createSession).mockRejectedValue(new Error('Connection failed'));
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Tool execution error: Connection failed');
  ...
  describe('manageSession', () => {
    it('approve_plan returns success', async () => {
      vi.mocked(client.approvePlan).mockResolvedValue(makeSession({ state: 'IN_PROGRESS' }));
  ...
  describe('getSessionStatus', () => {
    it('returns formatted status with nextSteps', async () => {
      vi.mocked(client.getSession).mockResolvedValue(
        makeSession({ id: 'sess-status', state: 'AWAITING_PLAN_APPROVAL', title: 'My Task' }),
      );

      const result = await tools.getSessionStatus({ session_id: 'sess-status' });
      const parsed = JSON.parse(result) as Record<string, unknown>;
  ...
  describe('waitForSession', () => {
    it('resolves when target state reached', async () => {
      const toolsPrivate = tools as unknown as { delay: (ms: number) => Promise<void> };
      ...
      vi.mocked(client.getSession)
        ...
        .mockResolvedValueOnce(makeSession({ id: 'sess-1', state: 'COMPLETED' }));
    ...
    it('throws timeout when state never reached', async () => {
      vi.useFakeTimers();
      const toolsPrivate = tools as unknown as { delay: (ms: number) => Promise<void> };
      vi.spyOn(toolsPrivate, 'delay').mockImplementation(
        (ms: number) => {
  ...
  describe('scheduleRecurringTask', () => {
    it('creates and schedules a task', async () => {
      ...
      vi.mocked(scheduler.getNextInvocation).mockReturnValue(new Date('2026-02-01'));
  ...
  describe('listSchedules', () => {
    it('returns formatted list', async () => {
      ...
      const parsed = JSON.parse(result) as { count: number; schedules: unknown[] };
  ...
  describe('getActivitiesSince', () => {
    it('returns activities for session', async () => {
      vi.mocked(client.listActivitiesSince).mockResolvedValue({
        activities: [{ type: 'PLAN_GENERATED', name: 'activities/1', timestamp: '2026-01-01T00:00:00Z' }],
      });
